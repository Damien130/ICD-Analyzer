...
import os
import cv2
import cv2.cuda as cv2_cuda # type: ignore
import sort
import csv
import time
...
from preProcessing import preProcessor_CV as pp
...
contrast_iterations = 4

# Specify the directory path containing the TIFF files
#background_path = '/mnt/d/Benam Lab/20230620/background/background.tif'

max_queue_length = 2
# created as global so individual batch won't require relearning
bg_subtractor = cv2_cuda.createBackgroundSubtractorMOG2(history=1240)
...
class YOLOv8_ObjectCounter(YOLOv8_ObjectDetector):
    ...
    def __init__(self, model_file = 'yolov8n.pt', labels= None, classes = None, conf = 0.01, iou = 0.20, 
                 track_max_age = 2484, track_min_hits= 1, track_iou_threshold = 0.1 ):
    ...
    def predict_video(self, image_folder, save_dir, batch_size):
        ...
        with ThreadPoolExecutor(max_workers=16) as executor:
            ...
            # Start a thread to read frames from the video file
            with tqdm(total=len(tiff_images), desc='Preprocessing', unit='image') as pbar:
                
                while image_index < len(tiff_images):
                    
                    # Refill buffer if needed
                    if len(futures) < max_queue_length:
                        image_name = tiff_images[image_index]
                        image_path = os.path.join(image_folder, image_name)
                        future = executor.submit(pp.process_image, image_path)
                        futures[image_index] = (image_name, future)
                        image_index += 1  # Increment image index only when a new future is submitted

                    # Check if any of the futures have completed
                    done_indexes = [index for index, (_, future) in futures.items() if future.done()]
                    for index in done_indexes:
                        name, future = futures.pop(index)

                        frames = future.result()
                        
                        for frame in frames: # Loop through the frames in the batch
                            frame_queue.put(frame)
                            pic = Image.fromarray(frame)
                            frame_path = os.path.join(save_dir, f'frame_{frame_counter}.png')
                            pic.save(frame_path)  # Saving the frame as a PNG file
                            frame_counter += 1

                        if (index + 1) % batch_size == 0:
                            frame_queue.put(None) # Put a None in the queue to indicate the end of a batch
                            self.process_queue(frame_queue, tracker, totalCount, currentArray, csv_file)
                            frame_counter = 0

                        # Update the progress bar
                        pbar.set_postfix({'Total Count': len(totalCount),
                                        'Cache Size': len([f for f in futures if f.done()])})
                        pbar.update(1)
                        break  # Break the inner loop after processing a future

    def process_queue(self, queue, tracker, totalCount, currentArray, csv_file):
        total_frames = queue.qsize()  # Get the total number of frames in the queue
        processed_frames = 0
        ...
        while True:
            ...
            processed_frames += 1
            progress_bar.set_postfix({'TotalCount': len(totalCount),
                                      'queue size': queue.qsize()})
            progress_bar.update(1)  # Update the progress bar

        progress_bar.close()  # Close the progress bar once processing is finished
